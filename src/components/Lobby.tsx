...
import { PlaySquare, Users, Video, LogOut, Settings, Upload, X, Search, Key, Plus, Lock, Globe, UserPlus, Check, XCircle, MonitorPlay, ChevronRight, User as UserIcon } from 'lucide-react';
...
export function Lobby({ onJoin, onWatchAnime, user, defaultUsername, defaultAvatar }: LobbyProps) {
  ...
  const generateFlagEmoji = (countryCode: string) => {
    ...
    const codePoint = countryCode.toUpperCase().split('').map(char => 127397 + char.charCodeAt(0));
  ...
  return (
    <div className="min-h-screen bg-[#0A0C10] text-zinc-100 font-sans flex flex-col items-center">
      ...
      <header className="w-full max-w-4xl px-4 py-6 flex items-center justify-between z-10">
        <button onClick={() => setActiveModal('profile')} className="flex items-center gap-3 hover:bg-[#11141A] p-2 rounded-2xl transition-colors text-left outline-none">
          ...
          {displayRooms.length === 0 ? (
            <div className="text-center py-12 bg-[#11141A] rounded-3xl border border-[#1F2937] border-dashed">
              ...
              {displayRooms.sort((a, b) => {
                ...
                return (
                  <motion.div whileHover={{ scale: 1.02 }} key={room.id} className="bg-[#11141A] border border-[#1F2937] hover:border-blue-500/50 rounded-2xl p-5 transition-all relative overflow-hidden group flex flex-col h-full cursor-default">
                    {hasFriend && (
                    ...
                    <div className="mt-auto pt-3 flex items-center justify-between border-t border-[#1F2937]">
                      ...
                      <div className="flex -space-x-2">
                         {room.users.slice(0, 5).map((u: any, i: number) => (
                           <button onClick={(e) => { e.stopPropagation(); openUserProfile(u.uid); }} key={i} title={u.username} className="w-6 h-6 rounded-full border-2 border-[#11141A] bg-[#1E293B] overflow-hidden flex items-center justify-center z-10 hover:scale-110 transition-transform" style={{ zIndex: 10 - i }}>
      ...
      <div className="fixed bottom-6 left-0 right-0 flex justify-center px-4 z-50">
        <div className="bg-[#11141A]/90 backdrop-blur-xl border border-[#1F2937] p-2 rounded-2xl shadow-2xl flex items-center gap-2">
          ...
          <button onClick={() => setActiveModal('settings')} className="p-3 bg-transparent hover:bg-[#1E293B] rounded-xl text-zinc-400 hover:text-white transition-all group" title="Настройки">
      ...
      <AnimatePresence>
        {activeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-sm bg-[#11141A] border border-[#1F2937] rounded-3xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
              <button onClick={() => setActiveModal(null)} className="absolute top-4 right-4 p-2 text-zinc-500 hover:text-zinc-300 bg-[#1E293B] rounded-full transition-colors z-10"><X className="w-4 h-4" /></button>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-center">
                       <label className="relative block w-24 h-24 rounded-3xl bg-[#1E293B] overflow-hidden cursor-pointer group border-2 border-[#1F2937] hover:border-blue-500 transition-colors">
                    ...
                    <div className="space-y-2">
                       ...
                       <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className="w-full bg-[#0A0C10] border border-[#1F2937] rounded-xl px-4 py-2 text-sm focus:border-blue-500/50 outline-none resize-none" placeholder="Расскажите о себе..." />
                    </div>
                    <div className="space-y-2">
                       ...
                       <select 
                          ...
                          className="w-full bg-[#0A0C10] border border-[#1F2937] rounded-xl px-4 py-2 text-sm focus:border-blue-500/50 outline-none appearance-none cursor-pointer"
                       >
                          {COUNTRIES.map(c => (
                             <option key={c.code} value={c.code}>
                                {c.code ? `${generateFlagEmoji(c.code)} ${c.name}` : c.name}
                             </option>
                          ))}
                       </select>
                    </div>
                ...
                <div className="space-y-6">
                  <div className="flex flex-col items-center">
                    <div className="relative mb-4 shadow-xl">
                       <div className="w-24 h-24 rounded-3xl bg-[#1E293B] border-2 border-[#1F2937] overflow-hidden">
                    ...
                    <h2 className="text-2xl font-bold">{viewingUser.username}</h2>
                    <div className="flex items-center gap-2 mt-1 px-3 py-1 bg-zinc-800/50 rounded-full border border-zinc-700/50">
                       {onlineUsersMap.includes(viewingUser.uid) ? (
                         <><span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span><span className="text-xs text-emerald-400">Онлайн</span></>
                       ) : (
                  ...
                  {viewingUser.country && (
                     <div className="bg-[#0A0C10] p-4 rounded-xl border border-[#1F2937] flex items-center gap-3">
                        <div>
                  ...
                  {viewingUser.description && (
                     <div className="bg-[#0A0C10] p-4 rounded-xl border border-[#1F2937]">
                        ...
                        {friendRequests.map(req => (
                          <div key={req.id} className="flex items-center justify-between p-3 bg-[#0A0C10] rounded-xl border border-[#1F2937]">
                            <button onClick={() => openUserProfile(req.from)} className="flex items-center gap-3 hover:opacity-80 transition-opacity outline-none text-left flex-1 min-w-0">
                               <div className="w-8 h-8 rounded-full bg-[#1E293B] overflow-hidden shrink-0">
                    ...
                    <div className="space-y-3 border-t border-[#1F2937] pt-4">
                      ...
                      <div className="max-h-40 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                        {friendsList.map(friend => {
                          ...
                          return (
                          <div key={friend.uid} className="flex items-center justify-between p-3 bg-[#0A0C10] rounded-xl border border-[#1F2937]">
