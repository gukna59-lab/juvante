...
export function Player({
  ...
  // Sync with server state for ReactPlayer
  useEffect(() => {
    const interval = setInterval(() => {
      if (!isCustomPlayer && playerRef.current && roomState.videoUrl && !isDraggingSeek) {
        ...
        // Prevents native scrolling inside the YouTube iframe from breaking state.
        if (diff > 2.5) {
  ...
  return (
    <div className="flex-none lg:h-full lg:flex-1 flex flex-col bg-black overflow-hidden relative">
      ...
      <div className={`flex-shrink-0 w-full z-10 bg-[#0F172A] border-t border-[#1F2937] ${isBannerVisible ? 'flex flex-col' : 'hidden lg:flex lg:flex-col'}`}>
        {/* Admin Setup Area / Queue form */}
        {isCreator && (
           <div className="p-4 bg-[#11141A] border-b border-[#1F2937]">
              <form onSubmit={handleUrlSubmit} className="flex relative max-w-xl mx-auto w-full">
                <input
                  type="text"
                  value={inputUrl}
                  onChange={(e) => setInputUrl(e.target.value)}
                  placeholder={roomState.videoUrl ? "Добавить видео в очередь..." : "Вставьте ссылку на видео (YouTube, Rutube, mp4)..."}
                  className="w-full bg-[#1E293B] border border-[#334155] rounded-l py-2 pl-4 pr-4 text-sm focus:outline-none focus:border-[#3B82F6] transition-colors text-[#CBD5E1] placeholder-[#475569]"
                />
                <button 
                  type="submit"
                  className="bg-[#3B82F6] hover:bg-[#2563EB] text-white px-4 py-2 rounded-r text-sm font-bold transition-colors"
                >
                  {roomState.videoUrl ? 'В очередь' : 'Загрузить'}
                </button>
              </form>
           </div>
        )}
        ...
        {roomState.queue && roomState.queue.length > 0 && (
           <div className="px-4 border-b border-[#1F2937] pb-2 pt-2 scrollbar-thin overflow-x-auto flex items-center gap-3">
             <div className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest shrink-0">Очередь:</div>
             {roomState.queue.map((q, i) => (
                <div key={i} className="flex items-center gap-2 bg-[#1E293B] border border-[#334155] rounded-lg px-2 py-1 shrink-0">
                  <span className="text-xs text-zinc-300 truncate max-w-[150px]">{q.title || q.url}</span>
                  {isCreator && (
                    <button onClick={() => onRemoveFromQueue?.(i)} className="text-red-400 hover:text-red-300 px-1 text-base">&times;</button>
                  )}
                </div>
             ))}
           </div>
        ...
        <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
                {isCreator ? (
                   ...
                   <span className="text-[10px] uppercase font-bold text-[#64748B] tracking-widest bg-[#1E293B] px-2 py-1 rounded border border-[#374151]">
                     Зритель
                   </span>
                )}
                
                {/* Reaction Buttons */}
                <div className="flex items-center gap-1 bg-[#1E293B] border border-[#334155] rounded px-1 py-1">
                  {['😂', '💔', '🔥', '👀', '💩', '🎉', '🤡'].map(emoji => (
                     <button
                        key={emoji}
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end">
                    {isCreator && (
                        <>
                            ...
                            {roomState.queue && roomState.queue.length > 0 && (
                              <button 
                                 onClick={() => onPlayNextQueue?.()}
                                 className="flex items-center gap-2 px-4 py-2 bg-[#10B981]/10 hover:bg-[#10B981]/20 border border-[#10B981]/30 text-[#10B981] text-[11px] font-bold rounded uppercase tracking-tighter"
                              >
        ...
        <div className="bg-[#0A0C10] p-4 border-t border-[#1F2937]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
             {users.map(user => {
                ...
                return (
                  <div key={user.id} className={`bg-[#11141A] p-2 rounded border ${isLagging ? 'border-yellow-500/50' : 'border-[#1F2937]'} flex flex-col group relative`}>
                    <span 
