...
async function startServer() {
  ...
  app.get('/api/users/:uid', (req, res) => {
    ...
    res.json({ uid: user.uid, username: user.username, avatar: user.avatar, description: user.description, country: user.country, lastOnline: user.lastOnline, friends: user.friends || [] });
  });
  ...
  app.post('/api/users/:uid/update', (req, res) => {
    ...
    localDb.saveUsers();
    res.json({ success: true, user: { uid: user.uid, username: user.username, login: user.login, avatar: user.avatar, description: user.description, country: user.country, friends: user.friends || [] } });
  });
  ...
  app.get('/api/users/search/:username', (req, res) => {
    ...
    if (!user) return res.status(404).json({ error: 'Пользователь не найден' });
    res.json({ uid: user.uid, username: user.username, avatar: user.avatar, description: user.description, country: user.country, lastOnline: user.lastOnline });
  });
